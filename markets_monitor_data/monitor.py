...
status_re=re.compile(r'<b>(.*?)</b')
name_re=re.compile(r'\.(.*?)\.(com|cn)')
...
def index():
    forex_status='获取最新状态失败，请手动点击链接进入'
    index_status='获取最新状态失败，请手动点击链接进入'
    try:
        r=requests.get('http://monitor.wallstreetcn.com/forex',timeout=2)
        forex_status=status_re.search(r.content).group(1)
        r=requests.get('http://monitor.wallstreetcn.com/index',timeout=2)
        index_status=status_re.search(r.content).group(1)
    ...
    for row in data:
        ...
        ctime=arrow.get(str(row['ctime']),'X').format('YYYY-MM-DD HH:mm:ss')
        site_ctime=arrow.get(str(row['site_ctime']),'X').format('YYYY-MM-DD HH:mm:ss')
