...
def forex():
    ...
    if good_count*1.0/all_count>forex_raito:
         status='总的状态:Good! 有效率:%.2f%% 允许误差时间:%d秒' %(good_ratio*100,diff_seconds)
    else:
        status='总的状态:Bad! 有效率:%.2f%% 允许误差时间:%d秒' %(good_ratio*100,diff_seconds)
...
def proxy():
    ...
    try:
        ...
        status='代理状态:Bad! 错误:%s' %(str(e))
    day_of_week,time_now=arrow.utcnow().replace(hours=8).format('d HHmmss').split(' ')
    day_of_week=int(day_of_week)
    time_now=int(time_now)
    if day_of_week==6 or day_of_week==7 or (time_now>=0 and time_now<80000):
        status='夜晚时间 代理状态:Good!'
